document.addEventListener("DOMContentLoaded", () => {
  ...
  const navClick = (e) => {
    ...
    #art1 문자열을 만들고 querySelector()로 art1 id 가 설정된 article box를
    selctor하고 article box 의 크기, 좌표 정보를 getter하고 그 정보에서
    top의 좌표만 추출

    window.scrollBy() 함수에 top은 article box 의 top으로 설정
    left는 화면 왼쪽 끝인 0으로 설정 smooth속성을 부여하여 자연스럽게
    화면이 스크롤 되도록 설정
    */
    if (tagName === "LI") {
      let className = e.target.className;

      // className = art1 -> #art1을 찾아라
      let art = document.querySelector("#" + className);

      // art 객체(DOM 객체)의 현재 위치, 모양값을 Getter
  ...
  event 핸들러에서 화면에 무언가 그리는 코드를 작성하게 되면 Browser, System에
  막대한 자원을 소모한다. 이러한 불편함을 없애기 위해 임의의 flag 변수를 하나
  만들고 이벤트가 시작되어 화면에 긜는 코드가 실행될 때 flag를 세팅하여
  다시 코드가 중복 실행되는 것을 방지
